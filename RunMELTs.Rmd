---
title: "Modeling Off-Axis Seamounts; Expediting insight into near-ridge mantle heterogeneity"
...
```{r plot}
plot(output$LaSmN, output$NbLa, col = 'black', pch = 20, xlab = "La/SmN", ylab = "Nb/La", xlim = c(0.5,4), ylim = c(0,2)) #create a plot of the model points, set axis limits and labels
...
```
## Next Steps
######Write several batch files to loop through and run the program multiple times in a row. Then loop through the outputs and plot them all together
######Use outputs to inform which parameters work best, and adjust accordingly and run again
######Use the models to constrain extents of melting and source contribution contributing to the seamount heterogeneity by plotting various trace element ratios of the models 
